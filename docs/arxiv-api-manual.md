...
tags:
  - "clippings"
---
[Skip to content](https://info.arxiv.org/help/api/#arxiv-api-users-manual)
...
[1\. Preface](https://info.arxiv.org/help/api/#_preface)  
[2\. API QuickStart](https://info.arxiv.org/help/api/#Quickstart)  
[3\. Structure of the API](https://info.arxiv.org/help/api/#Architecture)  
[3.1. Calling the API](https://info.arxiv.org/help/api/#_calling_the_api)  
[3.1.1. Query Interface](https://info.arxiv.org/help/api/#_query_interface)  
[3.1.1.1. search\_query and id\_list logic](https://info.arxiv.org/help/api/#search_query_and_id_list)  
[3.1.1.2. start and max\_results paging](https://info.arxiv.org/help/api/#paging)  
[3.1.1.3. sort order for return results](https://info.arxiv.org/help/api/#sort)  
[3.2. The API Response](https://info.arxiv.org/help/api/#api_response)  
[3.3. Outline of an Atom feed](https://info.arxiv.org/help/api/#atom_feed_outline)  
[3.3.1. Feed Metadata](https://info.arxiv.org/help/api/#_feed_metadata)  
[3.3.1.1. <title>, <id>, <link> and <updated>](https://info.arxiv.org/help/api/#_lt_title_gt_lt_id_gt_lt_link_gt_and_lt_updated_gt)  
[3.3.1.2. OpenSearch Extension Elements](https://info.arxiv.org/help/api/#_opensearch_extension_elements)  
[3.3.2. Entry Metadata](https://info.arxiv.org/help/api/#_entry_metadata)  
[3.3.2.1. <title>, <id>, <published>, and <updated>](https://info.arxiv.org/help/api/#title_id_published_updated)  
  
[3.3.2.3. <link>'s](https://info.arxiv.org/help/api/#entry_links)  
[3.3.2.4. <arxiv> extension elements](https://info.arxiv.org/help/api/#extension_elements)  
[3.4. Errors](https://info.arxiv.org/help/api/#errors)  
[4\. Examples](https://info.arxiv.org/help/api/#Examples)  
[4.1. Simple Examples](https://info.arxiv.org/help/api/#_simple_examples)  
[4.1.1. Perl](https://info.arxiv.org/help/api/#perl_simple_example)  
[4.1.2. Python](https://info.arxiv.org/help/api/#python_simple_example)  
[4.1.3. Ruby](https://info.arxiv.org/help/api/#ruby_simple_example)  
[4.1.4. PHP](https://info.arxiv.org/help/api/#php_simple_example)  
[4.2. Detailed Parsing Examples](https://info.arxiv.org/help/api/#detailed_examples)  
[5\. Appendices](https://info.arxiv.org/help/api/#Appendices)  
[5.1. Details of Query Construction](https://info.arxiv.org/help/api/#query_details)  
[5.1.1. A Note on Article Versions](https://info.arxiv.org/help/api/#_a_note_on_article_versions)  
[5.2. Details of Atom Results Returned](https://info.arxiv.org/help/api/#_details_of_atom_results_returned)  
[5.3. Subject Classifications](https://info.arxiv.org/help/api/#subject_classifications)

## 1\. Preface

The arXiv API allows programmatic access to the hundreds of thousands of e-prints hosted on [arXiv.org](http://arxiv.org/).

This manual is meant to provide an introduction to using the API, as well as documentation describing its details, and as such is meant to be read by both beginning and advanced users. To get a flavor for how the API works, see the [API Quickstart](https://info.arxiv.org/help/api/#Quickstart). For more detailed information, see [Structure of the API](https://info.arxiv.org/help/api/#Architecture).

For examples of using the API from several popular programming languages including perl, python and ruby, see the [Examples](https://info.arxiv.org/help/api/#Examples) section.

Finally, the [Appendices](https://info.arxiv.org/help/api/#Appendices) contain an explanation of all input parameters to the API, as well as the output format.

## 2\. API QuickStart

The easiest place to start with the API is by accessing it through a web browser. For examples of accessing the API through common programming languages, see the [Examples](https://info.arxiv.org/help/api/#Examples) section.
...
To learn more about how to construct custom search queries with the API, see the appendix on the [details of query construction](https://info.arxiv.org/help/api/#query_details). To learn about what information is returned by the API, see the section on [the API response](https://info.arxiv.org/help/api/#api_response). To learn more about writing programs to call the API, and digest the responses, we suggest starting with the section on [Structure of the API](https://info.arxiv.org/help/api/#Architecture).
...
2. If all goes well, the HTTP header will show a 200 OK status, and the response body will contain the Atom response content as shown in the [example response](https://info.arxiv.org/help/api/#response_example).
...
We have already seen the use of `search_query` in the [quickstart](https://info.arxiv.org/help/api/#Quickstart) section. The `search_query` takes a string that represents a search query used to find articles. The construction of `search_query` is described in the [search query construction appendix](https://info.arxiv.org/help/api/#query_details). The `id_list` contains a comma-delimited list of arXiv id's.
...
Detailed examples of how to perform paging in a variety of programming languages can be found in the [examples](https://info.arxiv.org/help/api/#detailed_examples) section.

In cases where the API needs to be called multiple times in a row, we encourage you to play nice and incorporate a 3 second delay in your code. The [detailed examples](https://info.arxiv.org/help/api/#detailed_examples) below illustrate how to do this in a variety of languages.
...
This can be very useful when implementing [paging of search results](https://info.arxiv.org/help/api/#paging). The other two elements `<opensearch:startIndex>`, and `<opensearch:itemsPerPage>` are analogous to `start`, and `max_results` [discussed above](https://info.arxiv.org/help/api/#paging).
...
If author affiliation is present, it is included as an `<arxiv:affiliation>` subelement of the `<author>` element as discussed [below](https://info.arxiv.org/help/api/#extension_elements).
...
Note that in this example, there are 3 category elements, one for each category. The first two correspond to arXiv categories, and the last one to an ACM category. See [<arxiv> extension elements](https://info.arxiv.org/help/api/#extension_elements) below for information on how to identify the arXiv primary category.
...
| [http://export.arxiv.org/api/query?start=not\_an\_int](http://export.arxiv.org/api/query?start=not_an_int) | `start` must be an integer |
| [http://export.arxiv.org/api/query?start=-1](http://export.arxiv.org/api/query?start=-1) | `start` must be >= 0 |
| [http://export.arxiv.org/api/query?max\_results=not\_an\_int](http://export.arxiv.org/api/query?max_results=not_an_int) | `max_results` must be an integer |
| [http://export.arxiv.org/api/query?max\_results=-1](http://export.arxiv.org/api/query?max_results=-1) | `max_results` must be >= 0 |
| [http://export.arxiv.org/api/query?id\_list=1234.1234](http://export.arxiv.org/api/query?id_list=1234.1234) | malformed id - see [arxiv identifier explanation](http://arxiv.org/help/arxiv_identifier#new) |
| [http://export.arxiv.org/api/query?id\_list=cond—mat/0709123](http://export.arxiv.org/api/query?id_list=cond%E2%80%94mat/0709123) | malformed id - see [arxiv identifier explanation](http://arxiv.org/help/arxiv_identifier#new) |
...
- [Perl](http://www.perl.org/) (via [LWP](http://search.cpan.org/~gaas/libwww-perl-5.808/lib/LWP.pm)) ([example](https://info.arxiv.org/help/api/#perl_simple_example))
- [Python](http://www.python.org/) (via [urllib](https://docs.python.org/3/library/index.html)) ([example](https://info.arxiv.org/help/api/#python_simple_example))
- [Ruby](http://www.ruby-lang.org/) (via [uri](https://ruby-doc.org/stdlib-2.5.1/libdoc/uri/rdoc/URI.html) and [net::http](https://ruby-doc.org/stdlib-2.7.0/libdoc/net/http/rdoc/Net/HTTP.html)) ([example](https://info.arxiv.org/help/api/#ruby_simple_example))
- [PHP](http://www.php.net/) (via file\_get\_contents()) ([example](https://info.arxiv.org/help/api/#php_simple_example))
...
| Perl | [XML::Atom](http://search.cpan.org/~miyagawa/XML-Atom-0.27/) | [parsing](https://info.arxiv.org/help/api/examples/perl_arXiv_parsing_example.txt) | [paging](https://info.arxiv.org/help/api/examples/perl_arXiv_paging_example.txt) |
| Python | [feedparser](https://github.com/kurtmckee/feedparser) | [parsing](https://info.arxiv.org/help/api/examples/python_arXiv_parsing_example.txt) | [paging](https://info.arxiv.org/help/api/examples/python_arXiv_paging_example.txt) |
| Ruby | [feedtools](http://sporkmonger.com/2005/08/11/tutorial) | [parsing](https://info.arxiv.org/help/api/examples/ruby_arXiv_parsing_example.txt) | [paging](https://info.arxiv.org/help/api/examples/ruby_arXiv_paging_example.txt) |
| PHP | [SimplePie](http://simplepie.org/) | [parsing](https://info.arxiv.org/help/api/examples/php_arXiv_parsing_example.txt) | [paging](https://info.arxiv.org/help/api/examples/php_arXiv_paging_example.txt) |

## 5\. Appendices

### 5.1. Details of Query Construction

As outlined in the [Structure of the API](https://info.arxiv.org/help/api/#Architecture) section, the interface to the API is quite simple. This simplicity, combined with `search_query` construction, and result set filtering through `id_list` makes the API a powerful tool for harvesting data from the arXiv. In this section, we outline the possibilities for constructing `search_query` 's to retrieve our desired article lists. We outlined how to use the `id_list` parameter to filter results sets in [search\_query and id\_list logic](https://info.arxiv.org/help/api/#search_query_and_id_list).
...
The following table lists each element of the returned Atom results. For a more detailed explanation see [Outline of an Atom Feed](https://info.arxiv.org/help/api/#atom_feed_outline).
...
For the complete list of arXiv subject classifications, please visit the page.
